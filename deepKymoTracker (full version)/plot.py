...
def create_color_dictionary(max_number_of_cells, N_cells): 
  base_colors=[[238,238,0,255],#cyan 1
...
def paste_benchmark_patch(patch,a,b,c,d,cell_number, frame_size):    
    ...
    return final_image
#######################################################
def plot_frame(cells,clip_centr,k,kk,fluor_images,names,out_folders,coords, coords_old, bright_images, bright_names, frame_size, n_digits, first_frame_number):      
       ...
       for kkk in range(len(cells)):               
         cv2.imwrite(os.path.join(out_folders[8],"segmented_patch_%s_cell_%s.tif") % (k+kk,kkk), cells["cell_%s" % kkk][3])# was [9]         
       ...
       cv2.imwrite(rename_file(out_folders[3],names[kk]),destin_fluor)#was [3]
       cv2.imwrite(rename_file(out_folders[9],bright_names[kk]),destin_bright)# was [9]
       ...
       #print("benchmark_seg=", benchmark_seg)
       #behchmark_seg ="mask"+str(k+kk+1).zfill(n_digits)+".tif"       
       cv2.imwrite(os.path.join(out_folders[0],benchmark_seg),destin_benchmark)

...
###############################################################
##################################################################################
def load_train_images(folder_dir):#,num): 
 raw_files_ch00=[]
 for filename in sorted_aphanumeric(os.listdir(folder_dir)):
      if filename.endswith(".tif"):
        ...
        raw.close()
 return raw_files_ch00#[:num]
##################################
def load_valid_images(folder_dir): 
 raw_files_ch00=[]
 for filename in sorted_aphanumeric(os.listdir(folder_dir)):
      if filename.endswith(".tif"):
        full_name=os.path.join(folder_dir, filename)
...
############################################################
"""
directory="/Users/kfedorchuk/Desktop/TRACKER_2_TRAIN"
subdirs = [x[1] for x in os.walk(directory)]
print("found  train folders", subdirs)
folders=subdirs[0]
"""
...
folders=["TRACKER_2_TRAIN/TWO_CELLS_Pos0101",
         "TRACKER_2_TRAIN/TWO_CELLS_Pos0501",
         "TRACKER_2_TRAIN/TWO_CELLS_Pos0203",
         "TRACKER_2_TRAIN/TWO_ART_CLOSE",
         "TRACKER_2_TRAIN/TWO_ART_DIST"]

folders_temp=[folders[0],folders[1], folders[2]]
#folders_temp=[folders[3],folders[4]]
#numbers=[738*5,738*5,738*5,1106*5,1106*5]
#numbers=[295*5,295*5,295*5,1771*5,1771*5]
#numbers=[100*5,100*5,100*5,2216*2*5,2216*2*5]
print("folders_temp=", folders_temp)


for i in range(len(folders_temp)):
     dirr=folders_temp[i]
     print(dirr)
     raw_files_ch00=load_train_images(dirr)#,numbers[i])
     n=len(raw_files_ch00)
     print("number of clips in this folder=",n/5)
     for filename in os.listdir(dirr):
        if filename.endswith(".npz"):
            data=np.load(os.path.join(dirr,filename))          
            new_centr=data["new_centr"]          
            print("loaded centroids from",os.path.join(dirr,filename))
            print("len(new_centr)=",len(new_centr)/5)   
     for k in range(0,n,5):
            film_0=[]
            for ii in range(5):
                frame=raw_files_ch00[k+ii].copy()            
...
directory="TRACKER_2_VALID"
subdirs = [x[1] for x in os.walk(directory)]
print("found  valid folders", subdirs)
folders=subdirs[0]
VALID=[]
...
for i in range(len(folders)):
     dirr=os.path.join(directory,folders[i])
     print(dirr)
     raw_files_ch00=load_valid_images(dirr)
     n=len(raw_files_ch00)
     print("number of images in this folder=",n)
     for filename in os.listdir(dirr):
        if filename.endswith(".npz"):
...
for k in range(1,N_frames+1):
    ...
    name = Dense(256,kernel_initializer='he_normal', bias_initializer='ones')(name)
    name=BatchNormalization()(name)
    name=Activation('relu')(name)
    name=Dropout((0.3))(name)
...
for k in range(1,N_frames+1):
    ...
    name = Dense(960,kernel_initializer='he_normal', bias_initializer='ones')(name)
    name=BatchNormalization()(name)
    name=Activation('relu')(name)
    #name=Dropout((0.3))(name)
...
def evaluate_train_and_save(model,train_samples, train_labels_mult):
   ...
   name_real_train_errors="TRACKER_2_TRAIN_ERRORS.json"
...
def evaluate_valid_and_save(model,valid_samples,valid_labels_mult):
   ...
   name_real_valid_errors="TRACKER_2_VALID_ERRORS.json"
...
for Epoch in range(NUM_EPOCHS):  
  ...
  if (Epoch==0 or SIGN > PREV_SIGN):       
       name_weights="TRACKER_2.weights.h5"  
...
"""
name_history="TRACKER-2-NEW-history-2.json"
#name_history="TRACKER_2_NEW_small_real_plus_2216*5.json"

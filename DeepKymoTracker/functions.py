...
######################################################## 
Bordersize=100
################## Apply Tracker-1  to predict centroids in Frame 1
...
def predict_tracking_general(coords,fluor_images,fluor_images_compressed,fluor_names,k,destination,trackers,n, cell_radius, frame_size): 
  ...
  tracker=trackers[N_cells-1]# choose tracker with regard to N_Cells in clip
...
def predict_tracking(coords,fluor_images,fluor_images_compressed,fluor_names,k,destination,tracker,n, cell_radius, frame_size):
   ...
   seed_empty=np.zeros((100,100), np.uint8)    
   coords_norm=np.zeros(coords.shape)  
   coords_norm[0,0]=coords[0][0]/frame_size*100
   coords_norm[0,1]=coords[0][1]/frame_size*100   
   markers=draw_exact_pixel_markers(seed_empty,coords_norm, int(cell_radius/frame_size*100))
   seed_frame=markers           
...
def extract_detected_intensities(final_list):
    ...
    return ints
#########################################################
def segment_patch(segmentor, refiner,empty_fluor,empty_bright,centroid,coord, cell_radius, frame_size):# segments frame and creates dictionary of cells         
...
def segment_and_clean(dict_of_divisions,cells,count,coords,prev_frame,text,segmentor, refiner,empty_fluor,empty_bright,centroids,frame_number, manual_id_indicator,mother_number, out_folders, cell_radius, frame_size, colours):
   ...
   for p in range(len(centroids)):# Step-1: segment based on tracking results
      ...
      coord=coords[p]
      # ensemble_outputs= list of 2 patches with the biggest and the 2nd bigest contours
      ensemble_output,a,b,c,d= segment_patch(segmentor, refiner,empty_fluor,empty_bright,centroid,coord, cell_radius, frame_size)     
   ...
   for pp in range(len(centroids)):      
      ...
      cell_parallel[c:d,a:b]=(image_template*2**patch[5])/1000000      
      init_seg+=cell
 ...
 #################### Step-2:try to improve segmentation of too small cells
   ...
   # clean: leave only n biggest contours in the frame
   ...
   for kkk in range(len(final_list)):
       ...
       cells["cell_%s" % number]=[segmentor_output,refiner_output,ensemble_output,sum_clean,empty_fluor_base,empty_bright_base,[x0,y0],a,b,c,d,txt, frame_number, prev_frame, coords, colour, division_indicator]                                                                                    
  ...
  if len(contours)==1:
     ...
     if flag=="second cleaning":
       ...
       return cleaned_patches# output is a list of 2 pathces with 2 separate biggest contours in each
        ...
        cleaned_patch=np.zeros((output.shape),dtype="uint8")       
        for iii in range(2):# leaves 2 biggest cells in a patch
          cv2.drawContours(cleaned_patch, [contours_separated[iii]] , 0, 255, -1)        
